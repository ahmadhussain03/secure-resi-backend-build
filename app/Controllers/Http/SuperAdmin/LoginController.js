...
const LoginValidator_1 = __importDefault(global[Symbol.for('ioc.use')]("App/Validators/SuperAdmin/LoginValidator"));
...
class LoginController {
    async index({ request, response, auth }) {
        ...
        const user = await User_1.default.query().where('username', data.username).where('user_type', UserType_1.UserType.super_admin).first();
        if (!user)
