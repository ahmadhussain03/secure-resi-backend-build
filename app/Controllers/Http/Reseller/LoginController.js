...
const LoginValidator_1 = __importDefault(global[Symbol.for('ioc.use')]("App/Validators/Reseller/LoginValidator"));
class LoginController {
    async index({ request, response, auth }) {
        const data = await request.validate(LoginValidator_1.default);
        const user = await User_1.default.query().where('username', data.username).where('user_type', UserType_1.UserType.reseller).first();
        if (!user)
