...
class ShiftRepository {
    ...
    async create(data) {
        const fromQuery = User_1.default.query();
        if (data.assignedItems?.length) {
            fromQuery.whereHas('items', (query) => {
                query.whereIn('id', data.assignedItems || []);
            });
        }
        const from = await fromQuery.where('id', data.from).preload('items').firstOrFail();
